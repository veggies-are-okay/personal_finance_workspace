"""Extract PURCHASE transactions from Chase credit-card PDF statements.
...
only ``app/``) can run it. The repo-root ``scripts/extract_chase_statements.py``
is a thin CLI wrapper that re-exports this module so the root project's tests
stay green.
...
# Repo root is four levels up: backend-python/app/ingestion/extract_chase.py.
...
def parse_lines(lines: list[str]) -> tuple[list[Transaction], Decimal | None]:
    """Parse already-extracted PDF text lines into purchases + summary total.

    Split out from :func:`parse_statement` so callers can feed synthetic line
    lists in unit tests without constructing a PDF.
    """
    full_text = "\n".join(lines)

    m = _OPEN_CLOSE_RE.search(full_text)
    if not m:
        raise ValueError("Could not find Opening/Closing Date in statement text")
...
def parse_statement(path: Path) -> tuple[list[Transaction], Decimal | None]:
    ...
    validate the parse.
    """
    return parse_lines(_pdf_lines(path))
