...
- [x] An onboarding/upload screen (drag-drop per source) posts to `/api/v1/ingest/{source}` and surfaces the summary; nginx routes `/api/v1/ingest/*` to the Python backend only (the TS-wired frontend still uploads to FastAPI for ingestion). *Done P8.2:* per-source `UploadControl` (drag-or-pick; `accept`/`multiple` per source) integrated into the Settings `SourceCard`s + a standalone `accounts` card; `api.ts` `ingestSource()` POSTs `multipart/form-data` (file field, no `Content-Type`) and maps the canonical error; on success the screen invalidates (`useApi keepDataOnReload`). `nginx.conf.template` adds a more-specific `location /api/api/v1/ingest/` → `backend-python` regardless of instance + `client_max_body_size 25m`.
  - *Verify:* ✅ upload a synthetic file in the UI → summary renders → dashboards reflect the new data; frontend gate. FE gate green (lint + Vitest **84 tests / 93% stmts**, ≥80% + build). E2e in an **isolated** compose project (`pf_p82`, own volume + alt ports) drove a synthetic `.csv` upload through the UI on **both** `:8511` and `:8512` (3 rows each → proving the TS-reads frontend's ingest still routes to backend-python); default stack/`pf_pgdata` volume never touched. Screenshot: `pull_requests/evidence/p8-2-upload-ui/proof.png`.
