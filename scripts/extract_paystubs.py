...
_PAIR_RX = {
    ...
    "ca_state_tax":       re.compile(rf"(?:State Withholding - CA|California State Tax)\s+{_MONEY}\s+{_MONEY}"),
}
...
COLUMNS = [
    "source_file", "employer", "period_start", "period_end", "pay_date",
    "gross_pay", "reimbursements", "deductions", "taxes", "employer_taxes", "net_pay",
    "gross_pay_ytd", "reimbursements_ytd", "deductions_ytd", "taxes_ytd",
    "employer_taxes_ytd", "net_pay_ytd",
    "federal_income_tax", "federal_income_tax_ytd",
    "medicare", "medicare_ytd",
    "social_security", "social_security_ytd",
    "ca_sdi", "ca_sdi_ytd",
    "ca_state_tax", "ca_state_tax_ytd",
    "retirement_401k_employee", "retirement_401k_employee_ytd",
    "retirement_401k_employer", "retirement_401k_employer_ytd",
    "medical_employee", "medical_employee_ytd",
    "salary", "salary_ytd", "salary_hours",
    "other_earnings", "other_earnings_ytd",   # bonuses, retro pay, etc. (= gross - base salary)
]
...
def parse_paystub(path: Path) -> dict:
    ...
    return parse_paystub_text(text, source_file=path.name,
                              employer=fm.group("emp").replace("-", " "),
                              period_start=start, period_end=end)


def parse_paystub_text(text: str, *, source_file: str, employer: str,
...
def main(argv: list[str] | None = None) -> int:
    ...
    for r in rows:
        ...
        print(f"{r['pay_date']:<12}{r['gross_pay']:>12,.2f}{r['net_pay']:>12,.2f}"
              f"{r['retirement_401k_employee']:>10,.2f}  {check}")

    write_csv(rows, args.out)
    latest = rows[-1]
    print(f"\nWrote {len(rows)} pay stubs to {args.out}")
    print(f"Latest YTD ({latest['pay_date']}): gross ${latest['gross_pay_ytd']:,.2f}, "
