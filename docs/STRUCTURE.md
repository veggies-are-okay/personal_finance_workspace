...
> - 2026-05-24: **Frontend upload UI + nginx ingest routing** (P8.2, FE). Added a per-source **file-upload control** to the Settings/Data Sources screen so the owner drops raw files into the Python-only ingest pipeline. `src/features/connections/UploadControl.tsx` (drag-or-pick file picker; `accept`/`multiple` per source: transactions `.csv,.pdf` ×N, income `.pdf,.csv` ×N, holdings `.csv`, accounts `.yaml,.yml`, loans `.csv`) renders idle/loading/**success** (per-file detected type + rows, `role="status"`)/**error** (canonical message, `role="alert"`). Integrated into `SourceCard` for the four connections sources; `accounts` (no Plaid row) gets a standalone card on `SettingsScreen`. `src/lib/api.ts` gained `ingestSource(source, files)` — POSTs `multipart/form-data` (files under the `file` field; **no `Content-Type`** so the browser sets the boundary) to `POST /api/v1/ingest/{source}`, maps the canonical error envelope to `ApiRequestError`; `types.ts` gained `IngestSource`/`IngestSummary`/`IngestedFile`; `mocks/handlers.ts` gained an ingest handler (demoable dev + `?scenario=error` 422). `useApi` gained `keepDataOnReload` so the success summary survives the post-upload invalidation refetch (no remount). **nginx (`nginx.conf.template`):** a more-specific `location /api/api/v1/ingest/` (before the general `/api/`) proxies to `http://backend-python:8000` **regardless of `${BACKEND_UPSTREAM}`** + `client_max_body_size 25m`, so ingestion from **both** frontends (incl. `:8502`/NestJS-reads) routes to FastAPI (NestJS implements no ingest). FE gate green (lint + Vitest 84 tests / 93% stmts ≥80% + build); e2e verified in an **isolated** compose project (`pf_p82`, own volume/ports) driving a synthetic upload through the UI on both `:8511`/`:8512` (3 rows each) — default stack/volume untouched. — P8.2.
...
│   │   │                      #       connections/ = Settings screen + Plaid Link flow (usePlaidConnect) + P8.2 UploadControl.tsx
│   │   │                      #       (per-source file-upload → Python-only POST /api/v1/ingest/{source})
...
│   │                          #     P8.2: more-specific /api/api/v1/ingest/ → backend-python ALWAYS (ingestion is Python-only) + 25m cap
